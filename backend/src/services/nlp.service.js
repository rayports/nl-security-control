...
const parseCommand = (text) => {
  ...
  if (intent === 'ADD_USER' || intent === 'REMOVE_USER') {
    ...
    const namePatterns = [
      ...
      /user\s+(?:named|called)\s+([A-Z][a-z]+)(?:\s+with|\s+pin|$)/i,
      // "add user John" or "create user Bob" - captures just the name before "with" or "pin"
      /(?:add|create)\s+user\s+([A-Z][a-z]+)(?:\s+with|\s+pin|$)/i,
      // "remove user John" - captures just the name
      /(?:remove|delete)\s+user\s+([A-Z][a-z]+)(?:\s+with|\s+pin|$)/i,
      // "add John with pin" or "remove John" - captures just the name
      /(?:add|remove|delete)\s+([A-Z][a-z]+)(?:\s+with|\s+pin|$)/i,
      // "John with pin" (standalone name before pin) - captures just the name
      /\b([A-Z][a-z]+)\s+with\s+pin/i
    ];
    ...
    for (const pattern of namePatterns) {
      ...
      if (match && match[1]) {
        ...
        if (!['add', 'remove', 'delete', 'create', 'user', 'named', 'called'].includes(potentialName.toLowerCase())) {
          // Split on "with" or "pin" to get just the name part
          const nameParts = potentialName.split(/\s+(?:with|pin)/i);
